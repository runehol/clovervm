...
namespace cl
{
        ...
        for(uint32_t idx = 0; idx < str->count.extract(); ++idx)
        {
            ...
            switch(ch)
            {
                ...
                default:
                    if(ch >= 0x20 && ch <= 0x7e)
                    {
                        out = fmt::format_to(out, "{}", static_cast<char>(ch));
                    }
                    else
                    {
                        out = fmt::format_to(out, "\\u{:04x}",
    ...
    template <typename Out> Out format_tuple_constant(Out out, Tuple *tuple)
    {
        ...
        for(size_t idx = 0; idx < tuple->size(); ++idx)
        {
            ...
            {
                fmt::format_to(out, "{}", item.get_smi());
            }
            else
            {
        ...
        -> decltype(ctx.out())
    {
        ...
        switch(intrinsic)
        {
            case cl::RuntimeIntrinsic0::Globals:
                return fmt::format_to(out, "Globals");
            case cl::RuntimeIntrinsic0::Locals:
                return fmt::format_to(out, "Locals");
            case cl::RuntimeIntrinsic0::ImportStar:
                return fmt::format_to(out, "ImportStar");
        }
...
template <> struct fmt::formatter<cl::Bytecode>
{
    ...
    auto format(const cl::Bytecode b, FormatContext &ctx) const
        -> decltype(ctx.out())
    {
        return fmt::format_to(ctx.out(), "{}", cl::bytecode_name(b));
    }
};
template <> struct fmt::formatter<cl::CodeObject>
{
    ...
    void disassemble_constant(const cl::CodeObject &code_obj, Out &out,
                              uint32_t pc) const
    {
        fmt::format_to(out, "c[{}]", code_obj.code[pc]);
    }
    ...
    void disassemble_read_cache(const cl::CodeObject &code_obj, Out &out,
                                uint32_t pc) const
    {
        fmt::format_to(out, "read_ic[{}]", code_obj.code[pc]);
    }
    ...
    void disassemble_mutation_cache(const cl::CodeObject &code_obj, Out &out,
                                    uint32_t pc) const
    {
        fmt::format_to(out, "mutation_ic[{}]", code_obj.code[pc]);
    }
    ...
    {
        fmt::format_to(out, "module_global_read_ic[{}]", code_obj.code[pc]);
    }
    ...
    disassemble_module_global_mutation_cache(const cl::CodeObject &code_obj,
                                             Out &out, uint32_t pc) const
    {
        fmt::format_to(out, "module_global_mutation_ic[{}]", code_obj.code[pc]);
    }
    ...
    void disassemble_function_call_cache(const cl::CodeObject &code_obj,
                                         Out &out, uint32_t pc) const
    {
        fmt::format_to(out, "call_ic[{}]", code_obj.code[pc]);
    }
    ...
    void disassemble_keyword_call_cache(const cl::CodeObject &code_obj,
                                        Out &out, uint32_t pc) const
    {
        fmt::format_to(out, "keyword_call_ic[{}]", code_obj.code[pc]);
    }
    ...
    void disassemble_operator_cache(const cl::CodeObject &code_obj, Out &out,
                                    uint32_t pc) const
    {
        fmt::format_to(out, "operator_ic[{}]", code_obj.code[pc]);
    }
                                 ...
                                 uint32_t pc) const
    {
        ...
        fmt::format_to(out, "{}", actual_target);
    }
                            ...
                            const cl::BytecodeInstruction &instruction) const
    {
        ...
        switch(bc)
        {
            ...
            case cl::Bytecode::CallMethodAttrPositional:
                ...
                fmt::format_to(out, ", ");
                fmt::format_to(out, "{}", code_obj.code[pc++]);
                break;
            ...
            case cl::Bytecode::CallMethodAttrKeyword:
                {
                    ...
                    fmt::format_to(out, ", kw_values=");
                    print_reg_span(out, code_obj, first_kw_value_reg,
                                   n_kw_args);
                    fmt::format_to(out, ", kw=");
            ...
            case cl::Bytecode::DictProbeStart:
                fmt::format_to(out, " ");
                disassemble_reg(code_obj, out, pc++);
                fmt::format_to(out, ", generation=");
                disassemble_reg(code_obj, out, pc++);
                fmt::format_to(out, ", hash_index=");
                disassemble_reg(code_obj, out, pc++);
                break;
            ...
            case cl::Bytecode::DictInsertNew:
                ...
                fmt::format_to(out, ", hash_index=");
                disassemble_reg(code_obj, out, pc++);
                fmt::format_to(out, ", first_tombstone_index=");
                disassemble_reg(code_obj, out, pc++);
                fmt::format_to(out, ", hash=");
                disassemble_reg(code_obj, out, pc++);
                fmt::format_to(out, ", key=");
                disassemble_reg(code_obj, out, pc++);
        ...
        -> decltype(ctx.out())
    {
        ...
        {
            fmt::format_to(out, "Exception table:\n");

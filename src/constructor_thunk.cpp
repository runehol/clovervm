...
namespace cl
{
    static void reserve_parameter_slots_and_frame_header(CodeObject *code)
    {
    ...
    static CodeObject *make_constructor_thunk_code(ClassObject *cls, Value init)
    {
        ...
        if(has_init)
        {
            ...
            code = make_object_raw<CodeObject>(
                init_code->compilation_unit, init_code->module_scope.extract(),
        ...
        {
            code = make_object_raw<CodeObject>(
                nullptr, active_vm()->get_builtin_scope().extract(),
                local_scope, thunk_name);
            code->n_parameters = 0;
        ...
        for(uint32_t param_idx = 0; param_idx < code->n_parameters; ++param_idx)
        {

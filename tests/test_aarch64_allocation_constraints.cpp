...
namespace cl::jit
{
         ...
         DefinesInitialGPRClassAndPlatformEntryABI)
    {
        ...
        EXPECT_EQ(24u, gprs.members().size());
        EXPECT_TRUE(gprs.members().contains(x(0)));
        EXPECT_TRUE(gprs.members().contains(x(15)));
        EXPECT_TRUE(gprs.members().contains(x(19)));
        EXPECT_TRUE(gprs.members().contains(x(20)));
        EXPECT_TRUE(gprs.members().contains(x(22)));
        EXPECT_TRUE(gprs.members().contains(x(24)));
        EXPECT_TRUE(gprs.members().contains(x(28)));
        EXPECT_FALSE(gprs.members().contains(x(16)));
        EXPECT_FALSE(gprs.members().contains(x(17)));
        EXPECT_FALSE(gprs.members().contains(x(18)));
        EXPECT_FALSE(gprs.members().contains(x(21)));
        EXPECT_FALSE(gprs.members().contains(x(25)));
        EXPECT_FALSE(gprs.members().contains(x(29)));
        EXPECT_FALSE(gprs.members().contains(x(30)));
        ASSERT_EQ(24u, gprs.allocation_order().size());
        EXPECT_EQ(x(0), gprs.allocation_order().front());
        EXPECT_EQ(x(28), gprs.allocation_order().back());

...
See [Refcounting and Reclamation](refcounting-and-reclamation.md) for the
...
Each slab maintains a valid-object bitmap as the authoritative set of
constructed heap object headers in that slab. One bit represents one possible
object header slot. For the ordinary 64 KiB slab and 32-byte heap pointer
granularity:
...
skipped by slab walks. Dedicated large-object slabs use the same bitmap API:
they have one valid object header slot, so bit 0 is the only meaningful bit.
The bitmap does not scale with dedicated slab byte size.
...
Bitmap indexing is allocation-lattice-relative:

```cpp
bit_index = (object_header - slab->first_object_header) >> 5;
```

Debug checks should enforce:
...
unmapping. Callers are responsible for checking releasability at explicit batch
points after they are done touching the slab:
...
This is thread-local allocator epoch state, not VM-global state. Presence in an
epoch list is distinct from being installed as an active allocation slab. Each
role owns its own pin. A slab can be active only, epoch-listed only, both, or
neither. In practice, newly installed ordinary active slabs should also be
remembered for the current epoch so allocations made from them can be discovered
later.
...
Dedicated large-object slabs do not count as ordinary inactive slabs. They
claim a slab on their own, are never installed as ordinary active allocation
slabs, and would distort the ordinary slab switch policy. Track them through
the shared epoch slab list and the dedicated byte counter. The epoch entry owns
the epoch discovery pin whether construction later succeeds or fails; failed
construction leaves no valid-object bit, so the normal epoch finish can drop
the discovery pin and ask `GlobalHeap` whether the slab is empty.
...
reclamation mechanism. They share lifecycle transitions, root filtering, native
descriptor teardown, and slab release batching.
...
### Reclamation Request Policy

Reclamation request policy can be refined after the mechanism is stable. Useful
triggers include:

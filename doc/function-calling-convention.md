...
JIT IR represents this distinction explicitly. `Return` consumes the result,
previous frame pointer, return code object, and return PC, then performs the
managed frame restoration above. `BareReturn` carries only a result and
performs no managed-frame restoration; it is used by code whose surrounding
ABI already owns that state.

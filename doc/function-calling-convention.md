...
In the current implementation, the Clover stack is not a C or C++ ABI stack.
Arbitrary native frames must never be placed on it. Future generated managed
code is designed to move Python frames onto a mixed managed/native stack while
keeping native frames opaque to managed root scanning. Native/managed transition
rules are defined separately in [Native/Managed Boundary
Contracts](native-managed-boundaries.md).
...
This split is a staging artifact, not the target architecture. Generated
managed code is designed to use the ordinary platform call stack with managed
frame headers laid out so the frame pointer and compiled return state occupy the
same positions expected by the host unwind convention. Once interpreter handlers
are generated by the shared backend, interpreted and compiled Python frames can
participate in the same mixed managed/native stack discipline.

Generated managed code must claim its frame storage by moving the architectural
stack pointer before writing durable managed state. GC-visible values,
interpreter-visible slots, frame headers, and call windows must not live in an
ABI red zone or any other area below the current stack pointer. Red-zone space,
when the target ABI provides it, is limited to non-root scratch state that is not
live across calls, safepoints, or stack walking. The architectural stack pointer
therefore serves as the lower frontier of generated managed frame storage.

The mixed-stack moving-GC contract is exact. Managed frame scanning is enabled
only for frames identified as generated managed frames by frame kind, code
metadata, or recognized native/managed transition thunks. Native C, C++,
extension, runtime-helper, host-ABI spill, and host-ABI return-address frames are
unwindable but opaque to managed root scanning. Any managed value that must live
across a native-opaque region is published through an exact transition record,
handle frame, or other explicitly walkable root mechanism before entering that
region.

Managed stack scanning must understand which managed slots are live values,
...
- C, C++, extension, and other host-ABI execution remains native-opaque to
  managed root scanning unless values are published through exact transition or
  handle records.
- Generated managed code moves the architectural stack pointer before storing
  durable managed frame state; no GC-visible state lives in an ABI red zone.

...
Public `__getitem__`, `get`, and `__contains__` general-key paths now use
generated bytecode with cache-bearing hash and equality sites. Mutation paths
still use C++ semantic drivers backed by `ThreadState::hash_value` and
`ThreadState::test_equal`. The remaining major work is to move those mutation
paths and general-key `CreateDict` into cache-bearing trusted bytecode and
complete the deliberately deferred mapping-method compatibility work. The
...
- [ ] Add structural codegen tests proving that insertion, deletion, and dict
  displays contain the intended hash/equality cache sites.

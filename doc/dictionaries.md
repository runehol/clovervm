...
operations on the canonical string-keyed shape bypass those general paths
through trusted protocol-free handlers or opcodes. The semantic C++ interface
remains available for native and C API callers and preserves the same promotion,
exception, and reentrant-equality behavior.

`dict.update` and exact builtin `dict` construction accept mappings, iterables
of pairs, and keyword arguments and populate through semantic insertion. The
former bootstrap `GeneralDict` class has been removed after its unique coverage
was migrated to public `dict`.

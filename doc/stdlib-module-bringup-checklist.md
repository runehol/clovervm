...
| [ ] | `errno` | Stable OS error constants. | Public `dict` now supports integer keys, so the `errno.errorcode` representation blocker is removed. Module constants, platform extraction, and compatibility tests remain unimplemented. |

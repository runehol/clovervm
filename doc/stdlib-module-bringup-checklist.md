...
| [~] | `glob` | Filesystem pattern expansion. | Pure-Python module now covers common POSIX string-path `glob`, list-backed `iglob`, `escape`, `has_magic`, deprecated-compatible `glob0`/`glob1` without warnings, hidden-file filtering, and a small recursive `**` directory expansion including zero-directory prefix matches. Missing bytes/path-like support, `root_dir`, `dir_fd`, exact generator behavior, symlink-specific recursion behavior, sorted/stable ordering guarantees, and full CPython `translate` semantics. |

...
| [~] | `operator` | Functional forms of operators used by collections, sorting helpers, libraries. | Pure-Python first slice covers supported arithmetic/comparison/identity/truth helpers, item get/set/delete, sequence concat via direct `__add__`, `contains`, `countOf`, `indexOf`, `length_hint`, `call` for up to three positional args, and aliases such as `__add__`. Unsupported helpers are explicit stubs where the VM lacks the needed operation/protocol: bitwise operators (`and_`, `or_`, `xor`, `invert`), power/matmul, callable factory helpers (`attrgetter`, `itemgetter`, `methodcaller`) because callable instances/closures are not ready, and exact `__all__` surface. |

...
- the operand shapes or exact types used by the dispatch decision
...
- lhs shape or exact type
- rhs shape or exact type
...
```text
(op, lhs shape/type, rhs shape/type, winner) -> BinaryHandler
```
...
if result is not NotImplemented:
    return result
otherwise:
    run full generic operator dispatch
```

The fallback must run the full generic dispatcher. It must not continue to a
cached second candidate, because the first arbitrary Python method may have
mutated globals, object state, classes, descriptors, bases, or MROs before
...
In-place operators use the same principle with one additional possible lookup:
...
The cache action must encode whether the first selected action is the in-place
method, the normal forward method, the reflected method, or a trusted handler.
If an arbitrary Python in-place method is first, the cache may call only that
method directly and must fall back to generic dispatch if it returns
`NotImplemented`.

A trusted-handler in-place cache is valid only when every skipped
dispatch-affecting fact is trusted and covered by the installed guards.
...
dispatch-affecting action is trusted and shape/type-stable.
...
- What exact runtime type should represent a cacheable special-method lookup
  result?

...
| Status | Accepted |
| Implementation | Partial; fixed `x19`, result constraints, canonical incoming stack locations, allocation-order register sets, per-class scratch registers, emitted side-exit transition references, and standalone interpreter/JIT entry thunks are implemented; fixed `x20` emission, interpreter call integration, stack-passed arguments, managed calls, and the side-exit thunk remain |
